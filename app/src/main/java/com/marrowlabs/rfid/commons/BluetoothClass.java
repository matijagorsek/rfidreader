...
/**
 * Created by Matija on 13-Oct-17.
 */
...
public class BluetoothClass {
    ...
    public BluetoothClass(Activity activity){
    ...
    private class ConnectThread extends Thread {
        public ConnectThread(BluetoothDevice device) {
            BluetoothClass.this.device=device;
            try {
                BluetoothClass.this.socket = device.createRfcommSocketToServiceRecord(MY_UUID);
            } catch (IOException e) {
                if(communicationCallback!=null)
                    communicationCallback.onError(e.getMessage());
            }
            ...
            switch (action) {
                case BluetoothAdapter.ACTION_STATE_CHANGED:
                    ...
                    if (state == BluetoothAdapter.STATE_OFF) {
                        if (discoveryCallback != null)
                            discoveryCallback.onError("BluetoothClass turned off");
                    }
                    break;
